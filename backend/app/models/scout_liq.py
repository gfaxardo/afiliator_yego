...
class CutoffScoutSummary(Base):
    ...
    cohort_target_count = Column(Integer, nullable=True)
...
class CutoffDriverLine(Base):
    ...
    # ── Rule traceability (Fase Motor Avanzado) ──
    rule_code = Column(String(100), nullable=True)
    rule_type = Column(String(50), nullable=True)
    origin_scope = Column(String(50), nullable=True)
    metric_code = Column(String(50), nullable=True)
    block_scope = Column(String(50), nullable=True)
    support_only = Column(Boolean, nullable=False, default=False, server_default=text("false"))
    # ── Metric window audit ──
    anchor_fallback_used = Column(Boolean, nullable=True)
    metric_window_start = Column(Date, nullable=True)
    metric_window_end = Column(Date, nullable=True)
    date_basis = Column(String(50), nullable=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class PaidHistory(Base):
    ...
    block_scope = Column(String(50), nullable=True)
    support_only = Column(Boolean, nullable=False, default=False, server_default=text("false"))
    scheme_version_id = Column(Integer, nullable=True)
...
class PaymentSchemeVersion(Base):
    ...
    block_scope = Column(String(50), nullable=True, default="driver_global", server_default=text("'driver_global'"))
    cohort_target_count = Column(Integer, nullable=True)
...
class ReconciliationRefreshLog(Base):
    __tablename__ = "scout_liq_reconciliation_refresh_log"
